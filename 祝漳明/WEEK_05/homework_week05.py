...
def main():
    ...
    for vector_index,lable in enumerate(kmeans.labels_):
        vector = vectors[vector_index] #句向量
        center = kmeans.cluster_centers_[label] # 类中心向量
        distance = cosin_distance(vector,center) #计算余弦距离
        density_dict[label].append(distance)
    for lable ,distance_list in density_dict.items():
        density_dict[lable] = np.mean(distance_list) #计算平均距离
    density_order = sorted(density_dict.items(),key=lambda x:x[1],reverse=True) # 排序
